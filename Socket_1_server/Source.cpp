...
int main()
{
	...
	local_addr.sin_port = htons(PORT);	// порт		Функция htons == Host TO Network Short (Изменяет порядок байт на сетевой (не прмяой)). Также есть версия ntohl (Network TO Host Long). + htonl, ntohs
	...
	char message[] = "Hello client!\r\n";	// Желательно завершать символами перевода каретки и строки.
